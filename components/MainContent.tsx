...
const MainContent: React.FC<MainContentProps> = ({ activeCategory, setActiveCategory, searchTerm, setSearchTerm, onCreateMarket }) => {
  ...
  return (
    <div className="flex-1 bg-[#f8f9fa] h-full overflow-y-auto">
      <FilterTabs 
      ...
      <div className="max-w-[1600px] mx-auto px-3 md:px-6 py-4 md:py-8 pb-24 md:pb-6 min-h-full bg-[#f8f9fa]">
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 gap-3 md:gap-6">
            {Array.from({ length: 8 }).map((_, index) => (
              <div key={index} className="p-5 bg-white rounded-xl border border-gray-200 animate-pulse">
          ...
          <>
            ...
            {displayMarkets.length === 0 && (
              <div className="text-center py-20">
                <div className="mx-auto w-24 h-24 bg-gray-800 dark:bg-gray-800 rounded-full flex items-center justify-center mb-6">
                ...
                <p className="text-gray-900">Try adjusting your filters or search terms.</p>
              </div>
            )}
